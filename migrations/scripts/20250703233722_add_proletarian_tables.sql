...
CREATE TABLE IF NOT EXISTS proletarian.job (
    ...
    attempts    INTEGER   NOT NULL DEFAULT 0, -- Number of attempts. Starts at 0. Increments when the job processed.
...
CREATE TABLE IF NOT EXISTS proletarian.archived_job (
    ...
    process_at  TIMESTAMP NOT NULL, -- Copied from job record (data from the last run only)
